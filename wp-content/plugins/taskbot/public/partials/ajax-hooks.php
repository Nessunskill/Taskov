        ...
        global  $taskbot_settings;
            ...
            if( !empty($type) ){
                ...
                if(!empty($type) && $type == 'post_task'){
                    ...
                    $_SESSION["redirect_url"]   = taskbot_get_page_uri('login');
...
if (!function_exists('taskbot_save_profile_settings')) {
    function taskbot_save_profile_settings()
    {
        ...
        if(!empty($hide_languages ) && $hide_languages == 'yes'){unset($list['english_level']);}
...
if (!function_exists('taskbot_get_terms_subcategories_dropdown')) {
    function taskbot_get_terms_subcategories_dropdown($parent_id, $term_ids=array())
    {
        if( !empty($parent_id) ){
            ...
            $taskbot_args   = array(
                'show_option_none'  => esc_html__('Task type', 'taskbot'),
                'show_count'    => 0,
                'hide_empty'    => 0,
            ...
            $child_categories        = '<label class="form-group-title">'.esc_html__('Task type:','taskbot').'</label><span class="tb-select">';
            $child_categories       .= wp_dropdown_categories( $taskbot_args );
            $child_categories       .= '</span></label>';
            echo do_shortcode( $child_categories );
    ...
    add_action( 'taskbot_get_terms_subcategories', 'taskbot_get_terms_subcategories_dropdown', 10, 2 );
}
