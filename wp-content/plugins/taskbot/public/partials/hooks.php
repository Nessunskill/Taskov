...
if (!function_exists('taskbot_custom_user_menu')) {
    add_filter('wp_nav_menu', 'taskbot_custom_user_menu', 10, 2);
        ...
        if (class_exists('ACF') && function_exists('get_field')) {
            ...
            if (!empty($user_details) && $user_details == 'yes') {
                if (is_user_logged_in()) {
                    ...
                    $items      .= '<div class="tk-navbarbtn"><a href="'.do_shortcode($login).'" class="tk-btn tk-login '.esc_attr($login_class).'">'.esc_html__('Sign in','taskbot').'</a><span data-type="post_task" id="tk_post_task" class="tk-btn-solid-lg">' . esc_html__('Post a task', 'taskbot') . '<i class="icon icon-plus"></i></span></div>';
                }
            }
        }
        return $items;
